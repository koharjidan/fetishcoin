...
// Copyright (c) 2013 Fetish developers
...
const string strMessageMagic = "FetishCoin Signed Message:\n";
...
static const int64 nStartSubsidy = 2.3 * COIN;
static const int64 nMinSubsidy = 1.15 * COIN;
...
else if(nHeight > 510000 && nHeight < 630000){
nSubsidy = 0.1 * COIN;

}
else if(nHeight > 630000 && nHeight < 900000){
nSubsidy = 0.01 * COIN;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FetishCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FetishCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("FetishCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running FetishCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FetishCoinMiner terminated\n");
        throw;
    }
}
